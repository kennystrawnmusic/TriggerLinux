...
su tempuser -c "yes | yay -Syu --devel yay-git archmaker plymouth-git snapd-glib-git snapd-git discover-snap ocs-url opencl-amd grub-git jade-application-kit-git pyside2 brave-bin ms-office-online"
