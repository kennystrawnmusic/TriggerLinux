...
compilecalamares() {
  ...
  if [ ! -d customrepo ]; then
    git clone git@github.com:realKennyStrawn93/triggerlinux-overlay customrepo
...
setuprepo() {
  cd customrepo
  git clone git@github.com:realKennyStrawn93/triggerlinux-gab
  cd triggerlinux-gab
  yes | makepkg -s --skipinteg || exit 1
  cp *.pkg.tar.* ../x86_64
  cd ..
  git clone git@github.com:realKennyStrawn93/triggerlinux-minds
  cd triggerlinux-minds
  yes | makepkg -s --skipinteg || exit 1
  cp *.pkg.tar.* ../x86_64
  cd ..
  git clone git@github.com:realKennyStrawn93/triggerlinux-parler
