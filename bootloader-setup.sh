...
default_append_line="root=/image.squashfs rootfstype=squashfs rootflags=loop init=/linuxrc ${cmdline_opts} ${custom_kopts} cdroot"
...
case ${clst_hostarch} in
	...
	ia64)
		...
		do
			...
			echo "  initrd=/efi/boot/${x}.igz" >> ${iacfg}
			echo >> ${iacfg}
			cp $1/boot/${x}{,.igz} $1/boot/efi/boot
		done
